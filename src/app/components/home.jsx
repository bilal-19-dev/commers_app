...
function Navbar({ setvalue }) {
  ...
  const logout = async () => {
    try {
      const res = await apiFetch(`https://${URL}/api/logout/`, {
        method: 'POST',
      });
      if (!res.ok) throw new Error();

      await fetch(`https://${URL}/api/token/`, {
        method: 'POST',
...
const Login = ({ setvalue, value, token }) => {
  ...
  const signIn = async () => {
    ...
    try {
      const res = await fetch(`https://${URL}/api/token/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const signUp = async () => {
    ...
    try {
      const res = await fetch(`https://${URL}/api/register/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const verifyCode = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch(`https://${URL}/api/VerifyOTPView/`, {
        method: 'POST',
        body: send_data,
      });
      if (!res.ok) {
