...
export default function Profile_component ({data}) {
    const [loding , setloding] = useState({value: true , hight : "356px" , width : "247.5px"})
    ...
    const [input, setinput] = useState(info.address_line && info?.phone_numbers?.length || 1);
    const [open, setopen] = useState("none");
    const [wilaya, setwilaya] = useState(
        info.address_line && info?.address_line?.wilaya !== "0"
            ? WILAYAS_DATA.find((w) => w.wilaya_name === info.address_line.wilaya)?.wilaya_code
            : "0"
    );
    ...
    const hendel_click = async () => {
        ...
        if (
            ...
            const valid = form.phone_numbers.every((p, idx) =>
                p && /^(05|06|07|02)\d{8}$/.test(p)
            );
            if (!valid) {
    ...
    return (
        <main>
            <div className="profile_contener">
                {loding.value === true ? (
                    <div className='loading_contener'><div className="loader"></div></div>
                ) : (
                    <>
                        <div className="contener" style={{display : open}}>
                            ...
                            <div className="edit_img">
                                <div className="img">
                                    <img src={form.image ? window.URL.createObjectURL(form.image) : info.image }></img>
                                </div>
                                <div className="input">
                            ...
                            {[...Array(input)].map((_, i) => (
                                <div key={i} style={{ display: "flex", alignItems: "center", margin: "2px" }}>
                                    <input
                                        type="text"
                                        value={form.phone_numbers[i] !== undefined ? form.phone_numbers[i] : ""}
                                        placeholder={info.phone_numbers && info.phone_numbers[i] ? info.phone_numbers[i] : `رقم الهاتف ${i + 1}`}
                                        onChange={e => {
                                    ...
                                    />
                                    {input > 1 && (
                                        <button type="button" className="remove" onClick={() => hendel_number_click("remove")}>
                            ...
                            <div className='wilaya'>
                                <label>Wilaya :</label>
                                <select
                                    value={wilaya !== undefined && wilaya !== null ? wilaya : (info.address_line.wilaya_code || "0")}
                                    onChange={(e) => {
                                        ...
                                        setform({
                                            ...
                                            address_line: {
                                                ...form.address_line,
                                                wilaya: WILAYAS_DATA.find(w => w.wilaya_code === e.target.value)?.wilaya_name || "0",
                                                baldya: ""
                                            }
                                        });
                                    }}
                                >
                                    <option value="0">
                                        {info.address_line && info.address_line.wilaya 
                                            ? `${info.address_line.wilaya_code || ''} - ${info.address_line.wilaya}`
                                            : "-- اختر الولاية --"}
                                    </option>
                                    {WILAYAS_DATA.map((w) => (
                            ...
                            <div className='wilaya'>
                                <label>Baldya :</label>
                                <select
                                    value={form.address_line.baldya !== undefined
                                        ? form.address_line.baldya
                                        : ""
                                    }
                                    disabled={wilaya === "0"}
                                    onChange={e => setform({
                                        ...
                                        address_line: {
                                            ...form.address_line,
                                            wilaya: WILAYAS_DATA.find(w => w.wilaya_code === wilaya)?.wilaya_name || "",
                                ...
                                >
                                    ...
                                    {wilaya !== "0" &&
                                        WILAYAS_DATA
                                            .find((w) => w.wilaya_code === wilaya)?.communes?.map((commune, idx) => (
                        ...
                        <div className="info">
                            <div className="img">
                                ...
                                <div>
                                    ...
                                    {info.checked ? <p className="status">status : <span>Active</span></p> : <p className="status">status : <span style={{ backgroundColor: 'red' }}>Disactive</span></p>}
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            setform({
                                                ...
                                                phone_numbers: Array.isArray(info.phone_numbers) ? info.phone_numbers.slice() : [],
                                            ...
                                            setwilaya(
                                                info.address_line.wilaya !== "0"
                                                    ? WILAYAS_DATA.find(w => w.wilaya_name === info.address_line.wilaya)?.wilaya_code || "0"
                                                    : "0"
                                            );
                                            setinput(Array.isArray(info.phone_numbers) ? info.phone_numbers.length : 1);
                                        }}
                                    >Edit...
                                        <span className="material-symbols-outlined">
                                            edit
                        ...
                        <div className="detail">
                            ...
                            {Array.isArray(info.phone_numbers) && info.phone_numbers.length > 0
                                ? info.phone_numbers.map((number, i) => (
                                    <p key={i}>{number}</p>
                                ))
